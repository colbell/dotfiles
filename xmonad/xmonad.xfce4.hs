...
-- #############################################################
-- #
-- # FOR running under XFCE4
-- #
-- # sudo vim /etc/xdg/xdg-xubuntu/xfce4/xfconf/xfce-perchannel-xml/xfce4-session.xml
-- # 
-- # Change xfce4wm to xmonad in Client0_Command.
-- # Change xfdesktop to #xfdesktop in Client3_Command as it closes teh XFCE4 panel.
-- #
-- #############################################################
...
myManageHook = manageScratchPad <+>composeAll (
    ...
    , resource  =? "whiskermenu-window" --> doFloat
    , title     =? "Whisker Menu"       --> doIgnore
    ])

myLayout = smartBorders $ showWName' mySWNConfig $
           desktopLayoutModifiers
           (mirrorTiled ||| tiled ||| Full ||| tabbed shrinkText myTabConfig)
    where
